...
export function validateState(data) {
 if(!data || data.version!==1 || !data.entries || typeof data.entries!=='object') throw new Error('Неподдерживаемый формат журнала. Нужен экспорт версии 1.');
 ...
 result.entries[a.id]={count:e.count,confirmed:e.confirmed,evidence:e.evidence.map(link=>{if(typeof link!=='string'||link.length>2048)throw new Error('Некорректная ссылка.');return safeUrl(link);})}; }
 return result;
}
